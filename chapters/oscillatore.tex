...
\begin{equation}
	\serz{n}a_n\sqrt{n}\ket{n-1}=\ser{n}a_n\sqrt{n}\ket{n-1}=\serz{m}a_{m+1}\sqrt{m+1}\ket{m}.
\end{equation}

Per l'ortonormalità degli autostati dell'hamiltoniano allora possiamo ricavare una relazione di ricorrenza per i coefficienti: per $n,m\in\N_0$\footnote{LA FORMULA È OTTENUTA MEDIANTE RIASSEGNAZIONE DEI NOMI DEGLI INDICI, I DELTA DI KRONECKER PRESENTI NON SONO UGUALI...RIGUARDARE.},
\begin{equation}
	\begin{aligned}
...
In una dimensione, con un potenziale della forma $V(q)= \frac{m \omega^2 q^2}{2}$, l'equazione di Schr\"odinger per la funzione d'onda $\psi$ nella base della posizione è
\begin{equation}
	\frac{\hbar^2}{2m}\psi''(q)+(E-\frac{m \omega^2 q^2}{2})\psi(q)=0.
...
\begin{equation}
	1=\norm{\psi_0}_2^2=\int_{-\infty}^{+\infty}\abs{\psi_0(y)}^2\,\dd y=A^2\int_{-\infty}^{+\infty}e^{-y^2}\,\dd y=A^2\sqrt{\pi}
\end{equation}
...
\begin{equation}
	\psi_n(y)=\frac{(-i)^n}{\sqrt{n!}}\bigg[-\frac{i}{\sqrt{2}}\bigg(\drv{}{y}-y\bigg)\bigg]^n\psi_0(y)=\frac1{\sqrt{2^nn!}}\bigg(y-\drv{}{y}\bigg)^n\psi_0(y)=\frac1{\sqrt{2^nn!}}\bigg(y-\drv{}{y}\bigg)^ne^\pi^{-\frac14}{-\frac{y^2}2}.
\end{equation}
...
Dato che $\psi_n\to 0$ per $\abs{y}\to+\infty$, possiamo approssimare l'equazione di Schr\"odinger da $-\psi_n''(y)+y^2\psi_n(y)=(2n+1)\psi_n(y)$\footnote{Sono state trascurate le costanti e manca un $\frac12$ che moltiplica il termine dato dagli autovalori.} a $-\psi_n''(y)+y^2\psi_n(y)=0$, e nel momento in cui trascuro il secondo membro l'equazione non dipende più da $n$.
...
Possiamo fare di meglio: aggiungiamo un termine polinomiale, con $\psi_n(y)=y^a\exp(-y^2/2)$\footnote{I passaggi di seguito non sono motivati e sembra ci sia nuovamente la mancanza del termine $\frac12$ nel termine degli autovalori}.
