...
Passiamo ora alla rappresentazione di Schr\"odinger (per funzioni in $L^2(\R^d)$, essendo la dimensione generica) in coordinate polari.
...
\end{equation}
siamo indotti a definire anche l'operatore coniugato a $\op r$, che è l'impulso radiale $\op p_r$ rappresentato in $L^2(\R^d)$ nelle coordinate sferiche da $-i\hbar\drp{}{r}$, in modo che $\scalar{\op{\vec x}}{\op{\vec p}}=\op r\op p_r$.
...
\end{equation}
dove $L$ è un opportuno operatore di $L^2(\R^d)$ che rappresenta il momento angolare orbitale.
