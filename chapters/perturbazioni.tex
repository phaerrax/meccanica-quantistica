...
Possiamo anche in questo caso immaginarci uno sviluppo formale di $\op{\mathbb U}$ del tipo $\op{\mathbb U}^{(0)}+\op{\mathbb U}^{(1)}+\cdots$, come spesso abbiamo già fatto per altre grandezze.
...
Al primo ordine, al primo membro troviamo $\drp{}{t}(\op{\mathbb U}^{(0)}+\op{\mathbb U}^{(1)})=\drp{}{t}(1+\op{\mathbb U}^{(1)})=\drp{}{t}\op{\mathbb U}^{(1)}$; al secondo membro, invece, abbiamo $\op{\mathbb V}(1+\op{\mathbb U}^{(1)})=\op{\mathbb V}+\op{\mathbb V}\op{\mathbb U}^{(1)}$ ma il termine $\op{\mathbb V}\op{\mathbb U}^{(1)}$ è già del secondo ordine, quindi è da trascurare.
